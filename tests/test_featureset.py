...
def test_feature_merging_order_invariance():
    ...
    assert_array_equal(merged_fs_shuf.vectorizer.get_feature_names_out().tolist(), feature_names)
...
def check_convert_featureset(from_suffix, to_suffix, with_labels=True):
    ...
    label_col = "y" if with_labels else None
    merged_exs = load_featureset(dirpath, featureset, to_suffix, label_col=label_col, quiet=True)

    # Load pre-merged data in the `to_suffix` format
    featureset = [f"{feature_name_prefix}{with_labels_part}_all"]
    premerged_exs = load_featureset(dirpath, featureset, to_suffix, label_col=label_col, quiet=True)
