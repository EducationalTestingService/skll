...
def check_filter_labels(inverse=False):
    ...
    if inverse:
        ids_kept = fs.ids[
            np.where(np.logical_not(np.in1d(fs.labels, labels_to_filter)))
        ]
    else:
...
def test_feature_merging_order_invariance():
    ...
    feature_names = (
        train_fs1.vectorizer.get_feature_names()
        + train_fs2.vectorizer.get_feature_names()
    )
    assert_array_equal(merged_fs.vectorizer.get_feature_names(), feature_names)
    assert_array_equal(merged_fs_shuf.vectorizer.get_feature_names(), feature_names)
    ...
    assert_array_equal(
        merged_fs.features[:, 0:2].todense(), train_fs1.features.todense()
    )
    assert_array_equal(
        merged_fs.features[:, 2:4].todense(), train_fs2.features.todense()
    )
    assert_array_equal(merged_fs.features.todense(), merged_fs_shuf.features.todense())

    assert not np.all(
        merged_fs.features[:, 0:2].todense() == merged_fs.features[:, 2:4].todense()
    )
...
def make_merging_data(num_feat_files, suffix, numeric_ids):
    ...
    for i in range(num_feat_files):
        ...
        subset_dict[str(i)] = [
            f"f{feat_num + j:03d}" for j in range(num_feats_per_file)
        ]
...
def make_conversion_data(num_feat_files, from_suffix, to_suffix, with_labels=True):
    ...
    if with_labels:
        label_map = {
            ...
            for num, label in enumerate(
                sorted(
                    {label for label in labels if not isinstance(label, (int, float))}
                )
            )
        }
    ...
    for i in range(num_feat_files):
        train_path = join(
            convert_dir, f"{feature_name_prefix}_{i}{with_labels_part}{from_suffix}"
        )
        sub_features = []
    ...
    train_path = join(
        convert_dir, f"{feature_name_prefix}{with_labels_part}_all{to_suffix}"
    )
    train_fs = FeatureSet(
...
def check_convert_featureset(from_suffix, to_suffix, with_labels=True):
    ...
    # Create test data
    make_conversion_data(
        num_feat_files, from_suffix, to_suffix, with_labels=with_labels
    )
    ...
    # format
    featureset = [
        f"{feature_name_prefix}_{i}{with_labels_part}" for i in range(num_feat_files)
    ]
    label_col = "y" if with_labels else None
...
def featureset_creation_from_dataframe_helper(with_labels, use_feature_hasher):
    ...
    if with_labels:
        ...
        expected = FeatureSet(
            featureset_name, ids, features=features, vectorizer=vectorizer
        )
...
def test_featureset_creation_from_dataframe_with_string_labels():
    ...
    dftest = pd.DataFrame(
        {"id": [1, 2], "score": ["yes", "no"], "text": ["a b", "b c"]}
    )
    dftest.set_index("id", inplace=True)
    test_feat_dict_list = [{"a": 1.0, "b": 1.0}, {"b": 1.0, "c": 1.0}]
    test_dict_vectorizer = DictVectorizer()
...
def test_reading_csv_and_tsv_with_drop_blanks():
    ...
    fs_csv = CSVReader(
        StringIO(test_csv), drop_blanks=True, pandas_kwargs=kwargs
    ).read()
    fs_csv.name = "test"

    fs_tsv = TSVReader(
        StringIO(test_tsv), drop_blanks=True, pandas_kwargs=kwargs
    ).read()
    fs_tsv.name = "test"
...
def test_reading_csv_and_tsv_with_fill_blanks():
    ...
    fs_csv = CSVReader(
        StringIO(test_csv), replace_blanks_with=4.5, pandas_kwargs=kwargs
    ).read()
    fs_csv.name = "test"

    fs_tsv = TSVReader(
        StringIO(test_tsv), replace_blanks_with=4.5, pandas_kwargs=kwargs
    ).read()
    fs_tsv.name = "test"
