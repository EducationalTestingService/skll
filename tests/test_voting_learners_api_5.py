...
class TestVotingLearnersAPIFive(unittest.TestCase):
    """Test class for fifth set of voting learner API tests."""
    ...
    def check_cross_validate_with_grid_search(
        self, learner_type, with_soft_voting, with_individual_predictions, with_custom_seed
    ):
        ...
        self.assertTrue(len(xval_results), 3)  # number of folds
        for i in range(3):
            if learner_type == "classifier":
            ...
            self.assertTrue(isinstance(xval_results[i][4], float))  # objective
            self.assertTrue(isinstance(xval_results[i][5], dict))  # metric scores

        # check that the expected fold IDs match the actual fold IDs
        self.assertEqual(sorted(expected_fold_ids.items()), sorted(used_fold_ids.items()))
        ...
        for (train, test), _ in zip(cv_splits, used_models):
            ...
            sklearn_model_type = (
                VotingClassifier if learner_type == "classifier" else VotingRegressor
            )
            ...
            if with_soft_voting:
                ...
                sklearn_preds_fold = np.array(
                    [sklearn_vl.classes_[x] for x in argmax_label_indices]
                )
            else:
        ...
        self.assertEqual(len(df_preds[df_preds["sklearn"].isnull()]), 0)
        ...
        if with_individual_predictions:
            for learner_name in learner_names:
                self.assertTrue(
                    Path(f"{prediction_prefix}_{learner_name}_predictions.tsv").exists()
                )

    def test_cross_validate_with_grid_search(self):
        for (
            learner_type,
            with_soft_voting,
            with_individual_predictions,
            with_custom_seed,
        ) in product(["classifier", "regressor"], [False, True], [False, True], [False, True]):
            # regressors do not support soft voting or custom seeds
            if learner_type == "regressor" and with_soft_voting:
                continue
            else:
                yield (
                    self.check_cross_validate_with_grid_search,
                    learner_type,
                    with_soft_voting,
                    with_individual_predictions,
                    with_custom_seed,
