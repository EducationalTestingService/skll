...
def test_config_parsing_no_name():
    ...
    config_path = fill_in_config_options(
        config_template_path, values_to_fill_dict, "no_name"
    )
...
def test_cv_folds_and_grid_search_folds():
    ...
    for (
        (task, cv_folds_or_file, grid_search_folds, use_folds_file_for_grid_search),
...
def check_cv_folds_and_grid_search_folds(
    ...
    fold_mapping = load_cv_folds(
        join(train_dir, "folds_file_test.csv"), ids_to_floats=False
    )
...
def test_config_parsing_param_grids_no_grid_search():
    ...
    with open(log_path) as f:
        warning_pattern = re.compile(
            r'Since "grid_search" is set to False, the specified "param_grids" will be'
            r" ignored."
        )
...
def test_config_parsing_no_grid_search_but_objectives_specified():
    ...
    with open(log_path) as f:
        warning_pattern = re.compile(
            r'Since "grid_search" is set to False, any specified "objectives" will be '
            r"ignored."
        )
        matches = re.findall(warning_pattern, f.read())
        eq_(len(matches), 1)
...
def test_config_parsing_param_grids_fixed_parameters_conflict():
    ...
    parse_config_file(config_path)
    log_path = join(
        output_dir, "config_parsing_param_grids_fixed_parameters_conflict.log"
    )
    with open(log_path) as f:
        warning_pattern = re.compile(
            r'Note that "grid_search" is set to True and "fixed_parameters" is also '
            r"specified. If there is a conflict between the grid search parameter "
            r"space and the fixed parameter values, the fixed parameter values will "
            r"take precedence."
        )
        matches = re.findall(warning_pattern, f.read())
        eq_(len(matches), 1)
...
def test_config_parsing_default_pos_label_str_value():
    ...
    config_path = fill_in_config_options(
        config_template_path, values_to_fill_dict, "default_value_pos_label_str"
    )
