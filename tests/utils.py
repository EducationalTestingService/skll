...
def make_classification_data(
    ...
    if one_string_feature:
        ...
        string_feature_column = np.array(string_feature_values, dtype=object).reshape(
            100, 1
        )
...
def make_sparse_data(use_feature_hashing=False):
    ...
    train_fs = FeatureSet(
        "train_sparse", ids, features=features, labels=y, vectorizer=vectorizer
    )
    ...
    test_fs = FeatureSet(
        "test_sparse", ids, features=features, labels=y, vectorizer=vectorizer
    )
