...
def test_compute_eval_from_predictions_with_probs():
    ...
    pred_path = join(
        other_dir, "test_compute_eval_from_predictions_probs_predictions.tsv"
    )
...
def test_compute_eval_from_predictions_breaks_with_expval_and_nonnumeric_classes():
    ...
    input_path = join(
        other_dir, "test_compute_eval_from_predictions_nonnumeric_classes.jsonlines"
    )
...
def test_conflicting_prediction_and_example_ids():
    ...
    input_path = join(
        other_dir, "test_compute_eval_from_predictions_different_ids.jsonlines"
    )
...
def check_generate_predictions(  # noqa: C901
    ...
    learner = Learner(
        learner_name, probability=enable_probability, pos_label_str=pos_label_str
    )
    ...
    gp_output_lines = _run_generate_predictions_and_capture_output(
    ...
    if use_regression or not enable_probability or use_threshold or predict_labels:
        ...
        if num_labels == 2:
            expected_header = ["id"] + [
                str(negative_class_label),
                str(positive_class_label),
            ]
        else:
    ...
    for (gp_line, expected_id, expected_prediction) in zip(
        ...
        if len(generated_fields) == 2:
            generated_id, generated_prediction = (
                generated_fields[0],
                generated_fields[1],
            )
...
def check_skll_convert(from_suffix, to_suffix, id_type):
    ...
    from_suffix_file = join(
        other_dir, f"test_skll_convert_{id_type}_ids_in{from_suffix}"
    )
    ...
    reader = EXT_TO_READER[to_suffix](
        to_suffix_file, ids_to_floats=ids_to_floats, quiet=True
    )
...
def check_print_model_weights(  # noqa: C901
    task="classification", sort_by_labels=False
):
    ...
    if task in [
        ...
        learner = Learner(
            "SVR", model_kwargs={"kernel": "linear"}, feature_scaling="both"
        )
        learner.train(train_fs, grid_search=True, grid_objective="pearson")
...
def test_plot_learning_curves_argparse():
    ...
    (
        ...
        keyword_arguments,
    ) = generate_learning_curve_plots_mock.call_args
...
def test_run_experiment_argparse():
    for (
        ...
        yield (
            check_run_experiments_argparse,
            multiple_config_files,
            n_ablated_features,
            keep_models,
            local,
...
def test_filter_features_no_arff_argparse():
    for (extension, filter_type, id_col, label_col, inverse, quiet) in product(
        ...
        yield (
            check_filter_features_no_arff_argparse,
            extension,
            filter_type,
            label_col,
            id_col,
...
def test_filter_features_arff_argparse():
    for (filter_type, label_col, id_col, inverse, quiet) in product(
        ...
        yield (
            check_filter_features_arff_argparse,
            filter_type,
            label_col,
            id_col,
            inverse,
...
def test_filter_features_unmatched_formats():
    ...
    # format
    for inext, outext in combinations(
        [".arff", ".ndj", ".tsv", ".jsonlines", ".csv"], 2
    ):
        ff_cmd_args = [f"foo{inext}", f"bar{outext}", "-f", "a", "b", "c"]
...
def check_join_features_argparse(extension, label_col="y", id_col="id", quiet=False):
    ...
    fs1, _ = make_classification_data(
        num_labels=3, train_test_ratio=1.0, random_state=1234
    )
...
def test_join_features_argparse():
    for (extension, label_col, id_col, quiet) in product(
        ...
        yield (check_join_features_argparse, extension, label_col, id_col, quiet)
