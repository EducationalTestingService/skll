...
def make_cv_folds_data(
    ...
    cv_fs = FeatureSet(
        "cv_folds", ids, features=features, labels=y, vectorizer=vectorizer
    )
...
def test_specified_cv_folds():
    ...
    for test_value, assert_func, expected_folds, use_hashing, use_sampler in [
        ...
        (grid_scores, _, _, _, _) = learner.cross_validate(
            cv_fs, cv_folds=folds, grid_search=True, grid_objective="f1_score_micro"
        )
...
def test_folds_file_logging_num_folds():
    ...
    with open(
        join(
            output_dir,
            "test_folds_file_logging_train_f0.jsonlines_LogisticRegression.log",
        )
    ) as f:
...
def test_folds_file_with_fewer_ids_than_featureset():
    ...
    with open(
        join(
            output_dir,
            "test_folds_file_logging_train_f5.jsonlines_LogisticRegression.log",
        )
    ) as f:
        cv_file_pattern = re.compile(
            r"Feature set contains IDs that are not in folds dictionary. Skipping "
            r"those IDs."
        )
        matches = re.findall(cv_file_pattern, f.read())
        eq_(len(matches), 1)
...
def test_cross_validate_task():
    ...
    with open(
        join(
            output_dir,
            "test_save_cv_folds_train_f0.jsonlines_LogisticRegression.results.json",
        )
    ) as f:
    ...
    skll_fold_ids_str = "".join(
        f"{key}{val}" for key, val in sorted(skll_fold_ids.items())
    )
