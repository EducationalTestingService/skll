...
def test_ablation_cv_all_combos():
    ...
    num_result_files = len(
        glob(join(output_dir, "ablation_cv_plain_all_combos*results"))
    )
...
def test_ablation_cv_feature_hasher():
    ...
    num_result_files = len(
        glob(join(output_dir, "ablation_cv_feature_hasher_*.results"))
    )
    eq_(num_result_files, 14)
...
def test_ablation_cv_feature_hasher_all_combos():
    ...
    config_template_path = join(
        config_dir, "test_ablation_feature_hasher_all_combos.template.cfg"
    )
    config_path = fill_in_config_paths(config_template_path)
    ...
    #    = 220 lines in total
    with open(
        join(output_dir, "ablation_cv_feature_hasher_all_combos_summary.tsv")
    ) as f:
        reader = csv.DictReader(f, dialect=csv.excel_tab)
        num_rows = check_ablation_rows(reader)
        eq_(num_rows, 220)
    ...
    # files
    num_result_files = len(
        glob(join(output_dir, "ablation_cv_feature_hasher_all_combos*.results"))
    )
    eq_(num_result_files, 20)
...
def test_ablation_cv_all_combos_sampler():
    ...
    config_template_path = join(
        config_dir, "test_ablation_sampler_all_combos.template.cfg"
    )
    config_path = fill_in_config_paths(config_template_path)
    ...
    # files
    num_result_files = len(
        glob(join(output_dir, "ablation_cv_sampler_all_combos*.results"))
    )
    eq_(num_result_files, 20)
...
def test_ablation_cv_feature_hasher_sampler():
    ...
    config_template_path = join(
        config_dir, "test_ablation_feature_hasher_sampler.template.cfg"
    )
    config_path = fill_in_config_paths(config_template_path)
    ...
    # make sure there are 7 ablated featuresets * 2 learners = 14 results files
    num_result_files = len(
        glob(join(output_dir, "ablation_cv_feature_hasher_sampler*.results"))
    )
    eq_(num_result_files, 14)
