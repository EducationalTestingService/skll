...
class TestOutput(unittest.TestCase):
    ...
    def make_learning_curve_data(self):
    ...
    # Function to make sure that the summary files contain the right results
    def check_summary_score(self, use_feature_hashing, use_additional_metrics):  # noqa: C901
        ...
        with open(output_dir / f"{outprefix}_summary.tsv") as f:
            ...
            for row in reader:
                ...
                if row["learner_name"] == "LogisticRegression":
                    ...
                    if use_additional_metrics:
                        summary_metrics_dict = literal_eval(row["additional_scores"])
                        nb_summary_additional_score1 = float(
                            summary_metrics_dict["unweighted_kappa"]
                        )
                    ...
                    if use_additional_metrics:
                        summary_metrics_dict = literal_eval(row["additional_scores"])
                        svm_summary_additional_score1 = float(
                            summary_metrics_dict["unweighted_kappa"]
                        )
                        svm_summary_additional_score2 = float(
                            summary_metrics_dict["f1_score_micro"]
                        )
    ...
    def test_summary(self):
        for use_feature_hashing, use_additional_metrics in product([True, False], [True, False]):
            yield self.check_summary_score, use_feature_hashing, use_additional_metrics

    def check_xval_fancy_results_file(
        self, do_grid_search, use_folds_file, use_folds_file_for_grid_search, use_additional_metrics
    ):
        ...
        if do_grid_search:
            self.assertEqual(results_dict["Grid Search"], "True")
            self.assertEqual(results_dict["Grid Objective Function"], "f1_score_micro")
        else:
            self.assertEqual(results_dict["Grid Search"], "False")
            self.assertTrue("Grid Search Folds" not in results_dict)
            self.assertTrue("Grid Objective Function" not in results_dict)

        if use_folds_file:
            self.assertEqual(results_dict["Number of Folds"], "5 via folds file")
            self.assertTrue("Stratified Folds" not in results_dict)
            self.assertEqual(results_dict["Specified Folds File"], str(folds_file_path))
            if do_grid_search:
                if use_folds_file_for_grid_search:
                    self.assertEqual(results_dict["Grid Search Folds"], "5 via folds file")
                    self.assertEqual(results_dict["Using Folds File for Grid Search"], "True")
                else:
                    self.assertEqual(results_dict["Grid Search Folds"], "4")
                    self.assertEqual(results_dict["Using Folds File for Grid Search"], "False")
        else:
            self.assertEqual(results_dict["Number of Folds"], "6")
            self.assertEqual(results_dict["Stratified Folds"], "True")
            self.assertTrue("Using Folds File for Grid Search" not in results_dict)
            self.assertTrue("Specified Folds File" not in results_dict)
            if do_grid_search:
                self.assertEqual(results_dict["Grid Search Folds"], "4")
    ...
    def test_xval_fancy_results_file(self):
        for (
            do_grid_search,
            use_folds_file,
            ...
            yield (
                self.check_xval_fancy_results_file,
                do_grid_search,
                use_folds_file,
                use_folds_file_for_grid_search,
                use_additional_metrics,
    ...
    def check_grid_search_cv_results(self, task, do_grid_search):
        ...
        if task in ["train", "predict"]:
            if do_grid_search:
                ...
                self.assertRaises(ValueError, run_configuration, config_path, quiet=True)
                # Short-circuit the test since a ValueError is expected and is fatal
                return
        else:
            run_configuration(config_path, quiet=True, local=True)
        ...
        for learner in learners:
            ...
            with open(expected_results_file_path) as expected, open(
                    ...
                    for expected_gs_cv_results, actual_gs_cv_results in zip(
                        ...
                        for field in ["grid_score", "grid_search_cv_results"]:
                            if do_grid_search:
                                ...
                                if field == "grid_score":
                                    assert (
                                        expected_gs_cv_results[field] == actual_gs_cv_results[field]
                                    )
                                else:
    ...
    def test_grid_search_cv_results(self):
        for task in VALID_TASKS:
            for do_grid_search in [True, False]:
                yield self.check_grid_search_cv_results, task, do_grid_search
    ...
    def check_learning_curve_implementation(self, with_probability):
    ...
    def test_learning_curve_implementation(self):
        yield self.check_learning_curve_implementation, False
        yield self.check_learning_curve_implementation, True
    ...
    def test_learning_curve_plots(self):
    ...
    def test_learning_curve_plots_with_objectives(self):
    ...
    def test_learning_curve_ylimits(self):
        ...
        self.assertEqual(len(ylimits_dict), 2)
        assert_almost_equal(ylimits_dict["neg_mean_squared_error"][0], -3.94, decimal=2)
        self.assertEqual(ylimits_dict["neg_mean_squared_error"][1], 0)
        self.assertEqual(ylimits_dict["r2"][0], 0)
    ...
    def check_pipeline_attribute(self, do_feature_hashing, learner, function_args_dict):
        ...
        if estimator_type == "classifier":
            ...
            self.assertEqual(metric_scores[metric], sklearn_score)
        else:
    ...
    def test_pipeline_attribute(self):
        ...
        for learner_name, do_feature_hashing, min_count, scaling_type, sampler_name in product(
            ...
            yield self.check_pipeline_attribute, do_feature_hashing, learner, function_args_dict

    def test_send_warnings_to_log(self):
    ...
    def test_save_models_to_current_directory(self):
        ...
        self.assertTrue(Path("test_current_directory.model").exists())
    ...
    def check_get_feature_names_out(self, selection):
    ...
    def test_get_feature_names_out(self):
        yield self.check_get_feature_names_out, True
        yield self.check_get_feature_names_out, False

    def test_get_feature_names_out_for_featurehasher(self):
