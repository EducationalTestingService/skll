...
def check_summary_score(use_feature_hashing, use_additional_metrics):  # noqa: C901
    ...
    if use_feature_hashing:
        ...
        summprefix = (
            "test_summary_with_metrics" if use_additional_metrics else "test_summary"
        )
    ...
    with open(join(output_dir, f"{outprefix}_LogisticRegression.results.json")) as f:
        ...
        if use_additional_metrics:
            ...
            logistic_result_additional_metric1 = results_metrics_dict[
                "unweighted_kappa"
            ]
        ...
        for row in reader:
            ...
            if row["learner_name"] == "LogisticRegression":
                ...
                if use_additional_metrics:
                    ...
                    nb_summary_additional_score2 = float(
                        summary_metrics_dict["f1_score_micro"]
                    )
    ...
    for result_score, summary_score, learner_name in test_tuples:
        assert_almost_equal(
            ...
            summary_score,
            err_msg=(
                f"mismatched scores for {learner_name} (result:{result_score}, "
...
def test_summary():
    ...
    for (use_feature_hashing, use_additional_metrics) in product(
...
def check_xval_fancy_results_file(
    ...
    config_path = fill_in_config_options(
        config_template_path, values_to_fill_dict, "xval"
    )
    ...
    results_file_path = join(
        output_dir, "test_fancy_xval_f0_LogisticRegression.results"
    )
...
def check_grid_search_cv_results(task, do_grid_search):  # noqa: C901
    ...
    exp_name = (
        f'test_grid_search_cv_results_{task}_{"gs" if do_grid_search else "nogs"}'
    )
    ...
    config_path = fill_in_config_options(
        config_template_path, values_to_fill_dict, task
    )
        ...
        with open(expected_results_file_path) as expected, open(
            ...
            if task == "cross_validate":
                ...
                for expected_line, actual_line in zip(expected_lines, actual_lines):
                    ...
                    assert (
                        expected_fields.intersection(actual_fields) == expected_fields
                    )
                    assert all(
...
def test_multiple_featuresets_and_featurehasher_throws_warning():
    ...
    config_template_path = join(
        config_dir, "test_warning_multiple_featuresets.template.cfg"
    )
...
def check_learning_curve_implementation(with_probability):
    ...
    learner = Learner(
        "MultinomialNB", min_feature_count=0, probability=with_probability
    )
    (train_scores2, test_scores2, train_sizes2) = learner.learning_curve(
        fs, cv_folds=cv_folds, train_sizes=train_sizes, metric="accuracy"
    )
...
def test_learning_curve_ylimits():
    ...
    ylimits_dict = _compute_ylimits_for_featureset(
        df_test, ["r2", "neg_mean_squared_error"]
    )
...
def test_learning_curve_min_examples_check_override():
    ...
    log_file_path = join(
        output_dir, "test_check_override_learning_curve_min_examples.log"
    )
...
def test_pipeline_attribute():
    ...
    for (
        ...
        scaling_type,
        sampler_name,
    ) in product(learners, use_hashing, min_feature_counts, scalers, samplers):
