...
def parse_config_file(config_path, log_level=logging.INFO):  # noqa: C901
    ...
    if config.has_option("General", "task"):
        ...
        raise ValueError(
            "Configuration file does not contain task in the [General] section."
        )
    ...
    custom_metric_path = locate_file(
        config.get("Input", "custom_metric_path"), config_dir
    )
    ...
    if feature_scaling not in VALID_FEATURE_SCALING_OPTIONS:
        raise ValueError(
            f"Invalid value for feature_scaling parameter: {feature_scaling}"
        )
    ...
    output_metrics = _parse_and_validate_metrics(
        output_metrics, "metrics", logger=logger
    )
    ...
    grid_objectives = _parse_and_validate_metrics(
        grid_objectives, "objectives", logger=logger
    )
    ...
    if task in ["learning_curve", "train"] and prediction_dir:
        raise ValueError(f"The predictions path should not be set when task is {task}.")
    if task == "learning_curve" and model_path:
        raise ValueError(
            "The models path should not be set when task is learning_curve."
        )
    ...
    if task == "cross_validate":
        cv_folds = (
            specified_folds_mapping if specified_folds_mapping else specified_num_folds
        )
