...
class SKLLConfigParser(configparser.ConfigParser):
    ...
    def _find_invalid_options(self) -> Set[str]:
        ...
        specified_options = set(
            itertools.chain(*[self.options(section) for section in self.sections()])
        )
    ...
    def validate(self) -> None:
        ...
        (
            incorrectly_specified_options,
            multiply_specified_options,
        ) = self._find_ill_specified_options()
        if multiply_specified_options:
            raise KeyError(
                "The following are defined in multiple sections: "
...
def parse_config_file(
    ...
    if config.has_option("General", "experiment_name"):
        ...
        raise ValueError(
            "Configuration file does not contain experiment_name " "in the [General] section."
        )
    ...
    # with a given name.
    logger = get_skll_logger("experiment", filepath=main_log_file, log_level=log_level)

    if config.has_option("General", "task"):
        task = config.get("General", "task")
    else:
        raise ValueError("Configuration file does not contain task in the " "[General] section.")
    if task not in VALID_TASKS:
        raise ValueError(
            f"An invalid task was specified: {task}.  Valid " f'tasks are: {", ".join(VALID_TASKS)}'
        )
    ...
    # is not set correctly
        ...
        logger.warning(
            "Ignoring hasher_features since feature_hasher is either" " missing or set to False."
        )
    ...
    if len(learners) == 0:
        raise ValueError(
            "Configuration file contains an empty list of learners" " in the [Input] section."
        )
    ...
    custom_learner_path = locate_file(config.get("Input", "custom_learner_path"), config_dir)
    ...
    # of features
    if not isinstance(featuresets, list) or not all(isinstance(fs, list) for fs in featuresets):
        raise ValueError(
    ...
    if featureset_names:
        if not isinstance(featureset_names, list) or not all(
    ...
    if len(learning_curve_cv_folds_list) == 0:
        ...
        if (
            not isinstance(learning_curve_cv_folds_list, list)
            or not all([isinstance(fold, int) for fold in learning_curve_cv_folds_list])
    ...
    if (
        not train_file
        and not test_file
        and (isinstance(featuresets, list) and len(featuresets) == 0)
    ):
    ...
    if not train_file and not train_path:
        raise ValueError(
            'Invalid [Input] parameters: either "train_file" or '
    ...
    if (task == "evaluate" or task == "predict") and not test_path:
        raise ValueError("The test set must be set when task is evaluate or " "predict.")
    ...
    if task in ["cross_validate", "train", "learning_curve"] and test_path:
        raise ValueError("The test set should not be set when task is " f"{task}.")
    ...
    if task in ["learning_curve", "train"] and prediction_dir:
        raise ValueError("The predictions path should not be set when task is " f"{task}.")
    if task == "learning_curve" and model_path:
        raise ValueError("The models path should not be set when task is " "learning_curve.")
    if task == "learning_curve":
        ...
        if len(output_metrics) == 0:
            raise ValueError(
                'The "metrics" option must be set when ' 'the task is "learning_curve".'
            )
    ...
    if task == "cross_validate":
        ...
        if save_cv_models is True and not model_path:
            raise ValueError(
                "Output directory for models must be set if " '"save_cv_models" is set to true.'
            )
