...
def main(argv=None):
    ...
    is_probabilistic_classifier = (
        hasattr(learner._model, "predict_proba") and learner.probability
    )
        ...
        if input_extension not in EXT_TO_READER:
            ...
            if args.threshold is not None:
                ...
                for neg_label_prob, pos_label_prob in original_predictions:
                    chosen_label = (
                        pos_label_str
                        if pos_label_prob >= args.threshold
                        else neg_label_str
                    )
            ...
            try:
                outputfh = (
                    open(args.output_file, "a") if args.output_file else sys.stdout
                )
