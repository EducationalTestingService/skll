...
def main(argv: Optional[List[str]] = None):
    ...
    parser.add_argument(
        "model_file", help="Model file to load and use for generating " "predictions."
    )
    parser.add_argument(
        "input_files",
    ...
    if estimator_type == "classifier":
        if len(learner.label_list) == 2:
            if learner.pos_label is not None:
                ...
                pos_label = [
                    label for label in learner.label_dict if learner.label_dict[label] == 1
                ][0]
            neg_label = [label for label in learner.label_list if label != pos_label][0]
            logger.info(f"{pos_label} is the label for the positive " "class.")
    ...
    for i, input_file in enumerate(args.input_files):
        ...
        if input_extension not in EXT_TO_READER:
            ...
            reader = EXT_TO_READER[input_extension](
                input_file, quiet=args.quiet, label_col=args.label_col, id_col=args.id_col
            )
            ...
            original_predictions = learner.predict(
                feature_set, class_labels=not learner.probability or args.predict_labels
            )
            ...
            if (
                ...
                or not learner.probability
                or args.predict_labels
