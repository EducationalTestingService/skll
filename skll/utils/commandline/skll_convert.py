...
def main(argv=None):
    ...
    if args.reuse_libsvm_map and output_extension == ".libsvm":
        ...
        for line in args.reuse_libsvm_map:
            ...
            feat_map.update(
                _pair_to_dict_tuple(pair) for pair in feat_map_str.strip().split()
            )
            label_map.update(
                _pair_to_dict_tuple(pair) for pair in label_map_str.strip().split()
            )
        feat_vectorizer = DictVectorizer()
        feat_vectorizer.fit([{name: 1} for name in feat_map])
        feat_vectorizer.vocabulary_ = feat_map
    else:
        feat_vectorizer = None  # BUG: Variable is not used.
        label_map = None
