...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument(
        ...
        dest="outfile",
        required=True,
        help="output feature file (must have same extension " "as input file)",
    )
    parser.add_argument(
        "-f",
        "--feature",
    ...
    parser.add_argument(
        "--inverse",
        help="Instead of keeping features and/or examples in " "lists, remove them.",
        action="store_true",
    )
    parser.add_argument(
        "-L",
    ...
    if input_extension == ".csv" or input_extension == ".tsv":
        ...
        kwargs = {"replace_blanks_with": replace_blanks_with, "drop_blanks": drop_blanks}
    else:
        kwargs = {}

    # Read input file
    reader = EXT_TO_READER[input_extension](
        args.infile, quiet=args.quiet, label_col=args.label_col, id_col=args.id_col, **kwargs
    )
    ...
    feature_set.filter(
        ids=args.id, labels=converted_labels, features=args.feature, inverse=args.inverse
    )
