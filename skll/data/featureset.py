...
class FeatureSet(object):
    ...
    def __add__(self, other):
        ...
        new_set = FeatureSet(
            "+".join(sorted([self.name, other.name])), deepcopy(self.ids)
        )
        ...
        if uses_feature_hasher:
            ...
            if set(self.vectorizer.feature_names_) & set(
        ...
        if self.has_labels:
            ...
            if other.has_labels and not np.all(
    ...
    def has_labels(self):
        ...
        if self.labels is not None and not all(label is None for label in self.labels):
            ...
            return not (
                np.issubdtype(self.labels.dtype, np.floating)
