...
class Reader(object):
    ...
    def _sub_read_rows(self, file):
        ...
        if total == 0:
            raise ValueError(
                f"No features found in possibly empty file '{self.path_or_list}'."
            )
    ...
    def _parse_dataframe(
        self, df, id_col, label_col, replace_blanks_with=None, drop_blanks=False
    ):
        """
    ...
    def read(self):
        ...
        if ids.shape[0] != len(set(ids)):
            raise ValueError(f"The example IDs are not unique in {self.path_or_list}.")
...
class LibSVMReader(Reader):
    ...
    def _sub_read(self, file):
        ...
        for example_num, line in enumerate(file):
            ...
            if not match:
                raise ValueError(f"Line does not look like valid libsvm format\n{line}")
                ...
                if match.group("feat_map"):
                    ...
                    for pair in match.group("feat_map").split():
                        ...
                        for (
                            orig,
                            replacement,
                        ) in LibSVMReader.LIBSVM_REPLACE_DICT.items():
...
class CSVReader(Reader):
    ...
    def _sub_read(self, file):
        ...
        """
        df = pd.read_csv(
            file, sep=self._sep, engine=self._engine, **self._pandas_kwargs
        )
        ...
        for example_num, row in enumerate(reader):
            if self.label_col is not None and self.label_col in row:
                class_name = safe_float(
                    row[self.label_col], replace_dict=self.class_map
                )
            ...
            if line:
                ...
                if row_type == "@attribute":
                    ...
                    if field_name == self.label_col:
                        self.regression = (
                            len(split_header) > 2 and split_header[2] == "numeric"
                        )
