...
class FilteredLeaveOneGroupOut(LeaveOneGroupOut):
    ...
    def split(self, X, y, groups):
        ...
        """
        for train_index, test_index in super(FilteredLeaveOneGroupOut, self).split(
            X, y, groups
        ):
...
def add_unseen_labels(train_label_dict, test_label_list):
    ...
    unseen_test_label_list = [
        label for label in test_label_list if label not in train_label_list
    ]
    ...
    unseen_label_dict = {
        label: i
        for i, label in enumerate(unseen_test_label_list, start=len(train_label_list))
    }
...
def get_acceptable_classification_metrics(label_array):
    ...
    # such as various correlations should work fine.
        ...
        label_type, (int, np.int32, np.int64, float, np.float32, np.float64)
    ):
...
def load_custom_learner(custom_learner_path, custom_learner_name):
    ...
    if not custom_learner_path.endswith(".py"):
        raise ValueError(f"custom_learner_path must end in .py ({custom_learner_path})")
        ...
        if learner.probability:
            try:
                ...
                learner.logger.error(
                    ...
                    f"{learner.model}\nProbability: {learner.probability}\n"
                )
                raise e
            else:
...
def setup_cv_fold_iterator(
    cv_folds, examples, model_type, stratified=False, logger=None
):
    """
...
def setup_cv_split_iterator(cv_folds, examples):
    ...
    featureset_iter = (
        FeatureSet.split_by_ids(examples, train, test) for train, test in cv_iter
    )
...
def train_and_score(learner, train_examples, test_examples, metric):
    ...
    if learner.model_type._estimator_type == "classifier":
        ...
        train_and_test_label_dict = add_unseen_labels(
            learner.label_dict, test_label_list
        )
...
def write_predictions(
    ...
    append=False,
    label_list=None,
    probability=False,  # BUG: This parameter seems not to be used.
):
    """
    ...
    with open(
        prediction_file, mode="w" if not append else "a", newline=""
    ) as predictionfh:
