...
class Learner(object):
    ...
    def __init__(  # noqa: C901
        ...
        self.pos_label_str = (
            safe_float(pos_label_str) if pos_label_str is not None else pos_label_str
        )
        self._model = None
        ...
        self._use_dense_features = issubclass(
            self._model_type, _REQUIRES_DENSE
        ...
        if model_kwargs:
            ...
            if (
                issubclass(
                    self._model_type,
                    (AdaBoostRegressor, AdaBoostClassifier, RANSACRegressor),
                )
    ...
    def from_file(cls, learner_path, logger=None):
        ...
        if not isinstance(learner, cls):
            raise ValueError(f"'{learner_path}' does not contain a learner object.")
    ...
    def model_params(self):
        ...
        elif isinstance(self._model, LinearSVC) or isinstance(
    ...
    def _create_estimator(self):
        ...
        if default_param_grid is None:
            raise ValueError(
                f"{self._model_type.__name__} is not a valid learner type."
            )
    ...
    def _create_label_dict(self, examples):
        ...
        if (
            self.label_dict is not None
            or self.model_type._estimator_type == "regressor"
        ):
            return
        ...
        if self.pos_label_str is not None:
            if len(self.label_list) != 2:
                ...
                self.label_list = sorted(
                    self.label_list, key=lambda x: (x == self.pos_label_str, x)
                )
    ...
    def _train_setup(self, examples):
        ...
        if (
            not issubclass(self._model_type, MultinomialNB)
            and self._feature_scaling != "none"
        ):
    ...
    def train(  # noqa: C901
        ...
        if grid_search:
            ...
            if estimator_type == "classifier":
                ...
                allowed_objectives = get_acceptable_classification_metrics(
            ...
            if (
                ...
                SCORERS[new_grid_objective] = make_scorer(
        ...
        if grid_search:
            ...
            if isinstance(grid_search_folds, int):
                ...
                fold_groups = [
                    grid_search_folds.get(curr_id, dummy_label)
                    for curr_id in examples.ids
                ]
    ...
    def evaluate(
        ...
        if estimator_type == "classifier":
            ...
            train_and_test_label_dict = add_unseen_labels(
                self.label_dict, test_label_list
            )
            ytest = np.array(
                [train_and_test_label_dict[label] for label in examples.labels]
            )
            acceptable_metrics = get_acceptable_classification_metrics(
                sorted_unique_labels
            )
    ...
    def predict(  # noqa: C901
        ...
        model_is_dict = isinstance(
            self.feat_vectorizer, (DictVectorizer, OldDictVectorizer)
        )
        model_is_hasher = isinstance(self.feat_vectorizer, FeatureHasher)
        data_is_dict = isinstance(
            examples.vectorizer, (DictVectorizer, OldDictVectorizer)
        )
        ...
        if both_dicts:
            if set(self.feat_vectorizer.feature_names_) != set(
    ...
    def cross_validate(
        ...
        if isinstance(cv_folds, dict):
            if (
                grid_search
                and use_custom_folds_for_grid_search
    ...
    def learning_curve(
        ...
        (featureset_iter, n_max_training_samples) = setup_cv_split_iterator(
            cv_folds, examples
        )
        ...
        out = parallel(
            joblib.delayed(train_and_score)(
                self, train_fs[:n_train_samples], test_fs, metric
            )
