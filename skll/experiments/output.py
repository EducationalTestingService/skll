...
def _compute_ylimits_for_featureset(df, metrics):
    ...
    for metric in metrics:
        ...
        train_values_lower = (
            df_train["value"].values - df_train["train_score_std"].values
        )
...
def generate_learning_curve_plots(experiment_name, output_dir, learning_curve_tsv_file):
    ...
    df_melted = pd.melt(
        df,
        id_vars=[
            c for c in df.columns if c not in ["train_score_mean", "test_score_mean"]
        ],
    )
    ...
    for fs_name, df_fs in df_melted.groupby("featureset_name"):
        ...
        with sns.axes_style(
            "whitegrid", {"grid.linestyle": ":", "xtick.major.size": 3.0}
        ):
            g = sns.FacetGrid(
                df_fs,
                row="metric",
            ...
            g = g.map_dataframe(
                sns.pointplot, "training_set_size", "value", scale=0.5, ci=None
            )
            ylimits = _compute_ylimits_for_featureset(df_fs, g.row_names)
            for ax in g.axes.flat:
                plt.setp(ax.texts, text="")
            g = g.set_titles(
                row_template="", col_template="{col_name}"
                        ...
                        if i == 0:
                            ...
                            plot_handles = [
                                matplotlib.lines.Line2D(
                                    [], [], color=c, label=l, linestyle="-"
                                )
                                for c, l in zip(
                                    colors, ["Training", "Cross-validation"]
                                )
                            ]
                            ax.legend(
...
def _print_fancy_output(learner_result_dicts, output_file=sys.stdout):
    ...
    if (
        ...
        print(
            "Using Folds File for Grid Search: "
    ...
    for lrd in learner_result_dicts:
        ...
        print(
            f'Grid Objective Score (Train) = {lrd.get("grid_score", "")}',
            file=output_file,
        )
        ...
        print(
            f'Objective Function Score (Test) = {lrd.get("score", "")}',
            file=output_file,
        )
...
def _write_learning_curve_file(result_json_paths, output_file):
    ...
    writer = csv.DictWriter(
        output_file, header, extrasaction="ignore", dialect=csv.excel_tab
    )
    writer.writeheader()
