...
def _check_job_results(job_results):
    ...
    for result_dicts in job_results:
        if not result_dicts or "task" not in result_dicts[0]:
            logger.error(f"There was an error running the experiment:\n{result_dicts}")
...
def _create_learner_result_dicts(
    ...
    for (
        ...
        zip(task_results, grid_scores, grid_search_cv_results_dicts), start=1
    ):
    ...
    if num_folds > 1:
        ...
        if result_table:
            ...
            result_table = tabulate(
                rows, headers=headers, floatfmt=".3f", tablefmt="psql"
            )
