...
def _classify_featureset(args):  # noqa: C901
    ...
    if args:
        raise ValueError(
            f"Extra arguments passed to _classify_featureset: {args.keys()}"
        )
    ...
    try:
            ...
            if not custom_metric_path:
                ...
                for custom_metric_name in possible_custom_metric_names:
                    try:
                        ...
                        logger.info(
                            f"registered '{custom_metric_name}' as a custom metric"
                        )
        ...
        if task == "cross_validate":
            ...
            logger.info(
                f"Cross-validating ({num_folds} folds) on "
                f"{train_set_name}, feature set {featureset} ..."
            )
        elif task == "evaluate":
            logger.info(
                f"Training on {train_set_name}, Test on "
        ...
        if (
            task in ["cross_validate", "learning_curve"]
        ...
        # load the model if it already exists
            ...
            if exists(modelfile) and not overwrite:
                logger.info(f"Loading pre-existing {learner_name} model: {modelfile}")
        ...
        if task == "cross_validate":
            ...
            if not exists(modelfile) or overwrite:
                ...
                if grid_search:
                    logger.info(
                        f"Best {grid_objective} grid search score: "
            ...
            param_out = (
                ...
                for param_name, param_value in learner.model.get_params().items()
            )
            ...
            res.update(
                {
                    ...
                    "learning_curve_train_scores_means": np.mean(
                        curve_train_scores, axis=1
                    ),
                    "learning_curve_test_scores_means": np.mean(
                        curve_test_scores, axis=1
                    ),
                    "learning_curve_train_scores_stds": np.std(
                        curve_train_scores, axis=1, ddof=1
                    ),
                    "learning_curve_test_scores_stds": np.std(
                        curve_test_scores, axis=1, ddof=1
                    ),
        ...
        # For all other tasks, i.e. train or predict
            ...
            if results_path:
                ...
                with open(results_json_path, "w") as json_file:
                    json.dump(
                        grid_search_cv_results_dict, json_file, cls=NumpyTypeEncoder
                    )
...
def run_configuration(  # noqa: C901
    config_file,
    local=False,
    overwrite=True,
    queue="all.q",
    ...
    try:
            ...
            for learner_num, learner_name in enumerate(learners):
                for grid_objective in grid_objectives:
                    ...
                    job_args["sampler_parameters"] = (
                        fixed_sampler_parameters if fixed_sampler_parameters else dict()
                    )
                    ...
                    job_args["param_grid"] = (
                        param_grid_list[learner_num] if param_grid_list else None
                    )
                    ...
                    job_args["learning_curve_cv_folds"] = learning_curve_cv_folds_list[
                        learner_num
                    ]
                        ...
                        jobs.append(
                            Job(
                                ...
                                num_slots=(
                                    ...
                                    if (do_grid_search or task == "learning_curve")
                                    else 1
                                ),
                                name=job_name,
                                queue=queue,
        ...
        if not local and _HAVE_GRIDMAP:
            if log_path:
                job_results = process_jobs(jobs, white_list=hosts, temp_dir=log_path)
            else:
        ...
        if (task == "cross_validate" or task == "evaluate") and write_summary:
            ...
            with open(
                join(results_path, summary_file_name), "w", newline=""
            ) as output_file:
