...
def _classify_featureset(args: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if args:
        raise ValueError("Extra arguments passed to _classify_featureset: " f"{args.keys()}")
    ...
    try:
            ...
            if not custom_metric_path:
                ...
                for custom_metric_name in possible_custom_metric_names:
                    try:
                        ...
                        logger.info(f"registered '{custom_metric_name}' as a " f"custom metric")
        ...
        if task == "cross_validate":
            ...
            logger.info(
                f"Cross-validating ({num_folds} folds, seed={cv_seed}) on "
            ...
            logger.info(f"Training on {train_set_name}, feature set " f"{featureset} ...")
        ...
        if isinstance(grid_search_folds, dict):
            grid_search_folds_to_print = f"{len(set(grid_search_folds.values()))} via folds file"
        else:
...
def run_configuration(
    ...
    try:
        ...
        if ablation is None or ablation > 0:
            ...
            raise ValueError(
                'Value for "ablation" argument must be either ' "positive integer or None."
            )
            ...
            for learner_num, learner_name in enumerate(learners):
                for grid_objective in grid_objectives_extra:
                    ...
                    job_args["fixed_parameters"] = (
                        fixed_parameter_list[learner_num] if fixed_parameter_list else dict()
                    )
                    job_args["param_grid"] = (
